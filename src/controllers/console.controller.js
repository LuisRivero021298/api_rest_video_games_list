...
const controller = {
  ...
  update: (req, res) => {
    if (!req.params.id) {
      return responseJson([res, 404, "Console not selected"]);
    }
  ...
  getConsolesByUser: (req, res) => {
    if (!req.userId) {
      return responseJson([res, 404, "Error: No token provided"]);
    }

    consoleModel
      .getConsoleByUser(req.userId)
      .then((consoles) => responseJson([res, 200, "", { consoles }]))
      .catch((err) => responseJson([res, 404, `Error: ${err}`]));
  },
  delete: (req, res) => {
...
function consoleInteraction(res, consoleId, action = "") {
  if (!consoleId) {
